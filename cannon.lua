...
function make_cannon()
    return {
        ...
        update = function(self)
            if btn(3) then self.angle = self.angle - 0.005 end
            if btn(2) then self.angle = self.angle + 0.005 end
            -- limit cannon angle
            ...
            if btnp(4) or btnp(5) then
                ...
                player = make_kitten(self.angle, self.x, self.y, self.length,
                                     shot_power)
            else
        ...
        calculate_current_power = function(self)
            ...
            if bar_rising then return power end
